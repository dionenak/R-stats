...
##In order to perform our tests, we have to remove the two unknown values in participants' scores. We didn't exclude the missing values in "gender" variable, because this variable is not relevant for our statistics analyses.
...
#Correlation
## A biserial correlation was computed between participants' scores and the time of the day that tasks were performed, either evening or morning, and two Pearson correlations between participants' age and scores, one for 
##morning scores and a second one for evening scores
...
#T-test
##A paired sample t-test examined the mean level differences on participants' scores as a function of time of the day. We used a paired t-test because same participants were examined in both evening and morning, so they
...
##We calculated the standardized effect size, delta hat  , for the difference of score means for morning and evening in that pilot study. Even though, it is a within-subject study, we calculated delta hats for both a 
...
#Power
##Three power analyses examined the number of participants needed to have eighty, ninety and ninety-five percent of power to detect the improvement in people's statistical skills in the evenings compared to their
##performance during the morning, using different effect sizes and assuming different study designs about the samples.

##Specifically, in the first power analysis, we assumed that the pilot study has a between-subjects design, using delta_IG=.55 and two-samples t-test analysis, at two sided 5% significant level, with 80%, 90% and 95% power.
...
##In the second power analysis, we assumed that the pilot study has a within-subjects design, using delta_IG=.55 and two-samples t-test analysis, at two sided 5% significant level, with 80%, 90% and 95% power.
...
## In the last power analysis performed, we assumed that the pilot study has a within-subjects design (paired t-test), using effect size for paired t-test, delta_RM=.73, at two-sided 5% significance level,
##with with 80%, 90% and 95% power. 
